...
class Bundler:
    ...
    def _strip_comments_block(string_css_or_js):
        while True:
            string_css_or_js, replaced = re.subn(r"^([^\"']*)/\*.*?\*/", r"\1", string_css_or_js, flags=re.DOTALL)  # strip block
            if replaced < 1:
                return string_css_or_js

    @staticmethod
    def strip_comments_from_css(string_css):
        string_css = Bundler._strip_comments_block(string_css)
        return string_css

    @staticmethod
    def strip_comments_from_js(string_js):
        string_js = Bundler._strip_comments_block(string_js)
        string_js = re.sub(r"^([^\"']*)//.*$", r"\1", string_js, flags=re.MULTILINE)  # strip end line
        return string_js
    ...
    @staticmethod
    def strip_line_from_css_or_js(string_css_or_js, marker):
        markerRE = re.compile("^.*" + marker + ".*$", flags=re.IGNORECASE)
        string_css_or_js = re.sub(markerRE, "", string_css_or_js, flags=re.MULTILINE)
        return string_css_or_js
    ...
    @staticmethod
    def fetch_external_js(bs4doc, path):
        for script in bs4doc.find_all('script', {'src': True}):
            src = script['src']
            if Bundler.src_is_external(src):
                continue
    ...
    @staticmethod
    def fetch_external_css(bs4doc, path):
